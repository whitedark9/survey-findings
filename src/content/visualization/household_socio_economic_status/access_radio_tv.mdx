...
# TABLE (2.12) ACCESS RADIO TV

<div className={"flex flex-col lg:flex-row items-start py-6 gap-4 lg:gap-0"}>

<SimpleBarChart data={data["household_socio_economic_status"]["access_radio_tv"]} />

<PieChartWithLabel
  data={data["household_socio_economic_status"]["access_radio_tv"]}
/>

</div>

<Callout type={"info"}>FINDINGS: {findings["access_radio_tv"]}</Callout>
