...
# TABLE (6.27) DEATH DUE TO EARTHQUAKE

<div className={"flex flex-col lg:flex-row items-start py-6 gap-4 lg:gap-0"}>

<SimpleBarChart data={data["deceased_persons"]["death_due_to_earthquake"]} />

<PieChartWithLabel
  data={data["deceased_persons"]["death_due_to_earthquake"]}
/>

</div>

<Callout type={"info"}>FINDINGS: {findings["death_due_to_earthquake"]}</Callout>
